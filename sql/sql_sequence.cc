...
// {min_value, max_value, ...}
bool sequence_definition::check_and_adjust(THD *thd, bool set_reserved_until, bool from_parser)
{
...
bool sequence_insert(THD *thd, LEX *lex, TABLE_LIST *org_table_list)
{
  ...
  */
  ...
  if (create_new)
  {
    ...
    if (seq->check_and_adjust(thd, true, true))
      DBUG_RETURN(TRUE);
  }
...
bool Sql_cmd_alter_sequence::execute(THD *thd)
{
  ...
  if (new_seq->check_and_adjust(thd, 0, true))
  {
